...
ship's 8-neighbour ring to `BLOCKED` (recoloring any earlier miss in the ring,
since it is the same guaranteed-empty water):
...
`HIT`, blocked cells are not fireable, an earlier miss inside the ring is
recolored to blocked, and a miss outside the ring is left untouched.
